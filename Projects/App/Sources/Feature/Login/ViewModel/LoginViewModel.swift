...
final class LoginViewModel: ObservableObject {
    ...
    @Published var loginPlatform: LoginPlatform = .apple
    ...
    // 여기서는 completionHandler (클로저)를 통한 비동기 방식으로 실행
    // 나는 여기에
    ...
    func createUserFirebase(nickname: String, profileImageURL: String, appsToken: String) async {
        do {
            try firebaseManager.createData(
                ...
                data: User(id: UUID().uuidString, nickname: nickname, profileImageURL: profileImageURL, appsToken: appsToken, createdDate: Date(), likeGameId: [], likeShopId: [])
            )
        } catch {
            print("\(error)")
        }
